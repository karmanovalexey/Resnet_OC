...
from resnet_ocr.bn_helper import BatchNorm2d, BatchNorm2d_class, relu_inplace
from resnet_ocr.backbone_utils import IntermediateLayerGetter
...
class ResNet_OCR(nn.Module):
    def __init__(self, config, backbone, last_inp_channels):
        ...
        self.aux_head = nn.Sequential(
            ...
            nn.ReLU(inplace=relu_inplace),
            nn.Conv2d(last_inp_channels, NUM_CLASSES,#config.DATASET.NUM_CLASSES,
    ...
    def forward(self, x):
        ...
        # x = F.interpolate(x, size=(x0_h, x0_w), mode='bilinear', align_corners=True)
        feats = x
...
def get_Resnet_OCR(cfg=None, **kwargs):
    backbone_name = 'resnet34'
    replace_stride_with_dilation = [False, False, False]

    backbone = torchvision.models.resnet.__dict__[backbone_name](pretrained=True,
                                                                 replace_stride_with_dilation=replace_stride_with_dilation)
    return_layers = {'layer3': 'out'}
    inplanes = 256

    backbone = IntermediateLayerGetter(backbone, return_layers=return_layers)
    model = ResNet_OCR(cfg, backbone, inplanes)
